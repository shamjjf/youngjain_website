...
export function PageHero({ title, subtitle, badge }: { title: React.ReactNode; subtitle?: string; badge?: string }) {
  return (
    <section style={{
      background: "linear-gradient(160deg, #081445 0%, #0F236A 40%, #1A3494 80%)", color: "#fff",
      padding: "160px 40px 80px", position: "relative", overflow: "hidden", minHeight: 340,
    }}>
      ...
      <div style={{ position: "absolute", top: "-20%", right: "-5%", width: 500, height: 500, borderRadius: "50%", background: "radial-gradient(circle, rgba(253,24,16,0.07) 0%, transparent 60%)", filter: "blur(60px)" }} />
      <div style={{ maxWidth: 1280, margin: "0 auto", position: "relative", zIndex: 2 }}>
        {badge && (
          <div style={{ display: "inline-block", padding: "6px 14px", borderRadius: 8, background: "rgba(253,24,16,0.15)", marginBottom: 20 }}>
            <span style={{ fontFamily: "var(--fb)", fontSize: 12, fontWeight: 700, color: "var(--red-l)", letterSpacing: 2, textTransform: "uppercase" }}>{badge}</span>
          </div>
        )}
        <h1 style={{ fontFamily: "var(--fh)", fontSize: 52, fontWeight: 300, lineHeight: 1.1, letterSpacing: -2 }}>{title}</h1>
        {subtitle && <p style={{ fontFamily: "var(--fb)", fontSize: 17, color: "rgba(255,255,255,0.5)", marginTop: 16, maxWidth: 600, lineHeight: 1.7 }}>{subtitle}</p>}
      </div>
    </section>
  );
}
...
export function SectionHeader({ badge, title, subtitle, center = true }: {
  badge?: string; title: React.ReactNode; subtitle?: string; center?: boolean;
}) {
  return (
    <div style={{ textAlign: center ? "center" : "left", marginBottom: 56 }}>
      {badge && (
        <div style={{ display: "inline-block", padding: "6px 14px", borderRadius: 8, background: "rgba(253,24,16,0.08)", marginBottom: 16 }}>
          <span style={{ fontFamily: "var(--fb)", fontSize: 12, fontWeight: 700, color: "var(--red)", letterSpacing: 2, textTransform: "uppercase" }}>{badge}</span>
        </div>
      )}
      <h2 style={{ fontFamily: "var(--fh)", fontSize: 44, fontWeight: 300, color: "var(--navy)", letterSpacing: -1.5 }}>{title}</h2>
      {subtitle && <p style={{ fontFamily: "var(--fb)", fontSize: 15, color: "var(--slate)", marginTop: 12, maxWidth: center ? 520 : undefined, margin: center ? "12px auto 0" : "12px 0 0", lineHeight: 1.7 }}>{subtitle}</p>}
    </div>
  );
}
      ...
      {items.map((item, i) => (
        <Reveal key={i} delay={i * 0.08}>
          <BentoCard title={item.title} desc={item.desc} tagColor={i % 2 === 0 ? "navy" : "red"}>
            <div style={{ fontSize: 36, marginTop: 12, opacity: 0.5 }}>{item.icon}</div>
          </BentoCard>
        </Reveal>
      ))}
    </div>
